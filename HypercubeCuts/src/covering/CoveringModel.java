...
public class CoveringModel implements MasterModel
{
	private Matrix M;
	...
	public CoveringModel(Matrix matrix)
	{
		M = matrix;
	}
	
	public CoveringModel(Matrix matrix, boolean integer)
	{
		M = matrix;
		_integer = integer;
	}
	...
	public void create()
	{
		try
		{
			...
			for(int i=0; i<M.elements(); ++i)
			{
				...
				for(int j=0; j<M.sets(); ++j) if( M.get(i,j) )
	...
	public void add(Inequality inequality)
	{
		...
		try
		{
			...
			for(int i=0; i<M.sets(); ++i) if( inequality.getLHS(i) != 0 )
